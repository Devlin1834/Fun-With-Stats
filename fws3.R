...
c.ex.sum <- sum(j.ex.table[1,])
c.ex.no <- j.ex.table[1,1] / c.ex.sum
c.ex.yes <- j.ex.table[1,2] / c.ex.sum
c.ex.no
c.ex.yes
...
c.value.sum <- sum(j.value.table[1,])
c.value.no <- j.value.table[1,1] / c.value.sum
c.value.yes <- j.value.table[1,2] / c.value.sum
c.value.no
c.value.yes
...
curve(expr = exp(ev.fit$coefficients[1] + ev.fit$coefficients[2] * x) / 
		 ...
		 col = 'firebrick', main = expression(logit(pi) == -0.9808 + 2.9178*x),
